...
本目录的 `genshen-cp2.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp2.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp2 deepking          # 导出 ~/.genshen-cp2/deepking/*.skin.json
  ```
...
```bash
genshen-cp2 deepking --check    # 应输出「通过」
genshen-cp2 deepking --what     # 看转换器实际会取到什么值
```
