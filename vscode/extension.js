...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-skin-CP2.git';
const EXT_ID = 'wp666.genshen-skin-cp2';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp2').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp2').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-skin-CP2'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-skin-CP2') : null,
...
function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshen-skin-cp2' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-skin-CP2') + ')'
  );
}
...
function activate(ctx) {
  ...
  reg('genshencp2.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshencp2.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP2 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshencp2.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshencp2')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshencp2.gallery', new GalleryProvider(), {
